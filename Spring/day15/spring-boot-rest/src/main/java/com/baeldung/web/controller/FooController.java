...
import com.baeldung.persistence.dto.FooDTO;
...
public class FooController {
    ...
    public Foo create(@RequestBody final FooDTO foo, final HttpServletResponse response) {
        Preconditions.checkNotNull(foo);
        Foo persistentFoo = new Foo();
        persistentFoo.setName(foo.getName());
        persistentFoo = service.create(persistentFoo);
        final Long idOfCreatedResource = persistentFoo.getId();
        eventPublisher.publishEvent(new ResourceCreatedEvent(this, response, idOfCreatedResource));
        return persistentFoo;
    }

    @PutMapping(value = "/{id}")
    @ResponseStatus(HttpStatus.OK)
    public void update(@PathVariable("id") final Long id, @RequestBody final FooDTO foo) {
        Preconditions.checkNotNull(foo);
        RestPreconditions.checkFound(service.findById(foo.getId()));
        Foo persistentFoo = new Foo();
        persistentFoo.setId(foo.getId());
        persistentFoo.setName(foo.getName());
        service.update(persistentFoo);
    }
