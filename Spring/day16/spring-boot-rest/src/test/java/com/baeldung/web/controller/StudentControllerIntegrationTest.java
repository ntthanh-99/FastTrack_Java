...
public class StudentControllerIntegrationTest {
    ...
    @Test
    public void whenReadOne_thenStatusIsNotFound() throws Exception {
        when(studentService.read((long) 1)).thenReturn(null);
        this.mockMvc.perform(get(STUDENTS_PATH + 10))
    ...
    @Test
    public void whenUpdate_thenStatusIsNotFound() throws Exception {
        Student student = new Student(1, "Nikola", "Tesla");
        when(studentService.update(student.getId(), student)).thenReturn(null);
        this.mockMvc.perform(put(STUDENTS_PATH + 10)
                        .content(asJsonString(student))
                        .contentType(MediaType.APPLICATION_JSON_VALUE))
                        .andExpect(status().isNotFound());
    }
