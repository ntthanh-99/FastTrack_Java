...
import com.spring.batch.processor.AddressProcessor;
...
public class UserAddressJob {
    ...
    public FlatFileItemReader<AddressModel> reader() {
        ...
        reader.setResource(new ClassPathResource(filePath + "/user_address.csv"));
        reader.setLinesToSkip(1);
        reader.setLineMapper(new DefaultLineMapper<AddressModel>() {{
            setLineTokenizer(new DelimitedLineTokenizer() {{
                setNames("name", "address1", "address2", "zip_code", "city", "country");
            }});
    ...
    public JdbcBatchItemWriter<AddressDTO> writer() {
        ...
        writer.setSql("INSERT INTO ADDRESS (NAME,ADDRESS1,ADDRESS2,ZIP_CODE,CITY,COUNTRY) " + "VALUES (:name,:address1,:address2,:zipCode,:city,:country)");
    ...
    public Job addressJob(JobListener listener) {
        return jobBuilderFactory.get("addressJob").incrementer(new RunIdIncrementer()).listener(listener).flow(importData()).end().build();
    }
    ...
    public Step importData() {
        return stepBuilderFactory.get("AddressJobStep").<AddressModel, AddressDTO>chunk(10).reader(reader()).processor(processor()).writer(writer()).build();
    }
